...
if ( ! defined( 'SED_EDITOR_DIR' ) )
	define( 'SED_EDITOR_DIR', SED_PLUGIN_DIR . DS .'editor' );

if ( ! defined( 'SED_INC_EDITOR_DIR' ) )
    define( 'SED_INC_EDITOR_DIR', SED_EDITOR_DIR . DS .'includes' );

if ( ! defined( 'SED_EDITOR_FOLDER_URL' ) )
	define( 'SED_EDITOR_FOLDER_URL', SED_PLUGIN_URL . '/editor/' );

if ( ! defined( 'SED_FRAMEWORK_DIR' ) )
    define( 'SED_FRAMEWORK_DIR', SED_PLUGIN_DIR . DS . 'framework' . DS . 'framework' );
...
include_once SED_PLUGIN_DIR . DS . 'includes' . DS . "editor-extensions.class.php";
...
class SEDAppInit
{
    ...
    function __construct()
    {
        ...
        if( is_admin() ){
            if( !( is_site_editor() ||  (isset( $_POST['sed_app_editor'] ) && $_POST['sed_app_editor'] == "on") ) ) {
                $this->backend();
            }
        }else{
            $this->front_end();
        ...
        if( SiteEditorSetup::is_installed() ) {
            ...
            if( is_admin() && ( is_site_editor() || (isset( $_POST['sed_app_editor'] ) && $_POST['sed_app_editor'] == "on") ) ) {
                ...
                add_action('current_screen', array(//current_screen
                    &$this, 'sed_admin_init',
                ));
            }
        }
    ...
    function sed_admin_init(){
        //if( is_site_editor() || (isset( $_POST['sed_app_editor'] ) && $_POST['sed_app_editor'] == "on") ){
        load_page_builder_app();
        global $sed_apps;
        $sed_apps->load_site_editor_app();

        require_once SED_EDITOR_DIR . DS . 'index.php';
    ...
    */ 
    ...
    function render_portfolio(){
        include_once SED_PLUGIN_DIR . DS . 'framework' . DS . 'portfolio.php' ;
    }
...
if(isset( $_POST['sed_page_ajax'] ) ){
    require_once SED_PLUGIN_DIR . DS . 'framework' . DS . 'sed_ajax.php';
}
    ...
    */
    ...
    function __construct($app_name,$option_settings,$domain,$lang_rel_path) {
        ...
        if( site_editor_app_on() ){
            require_once SED_EDITOR_DIR . DS . 'libraries' . DS . 'siteeditor' . DS . 'site-iframe' . DS . 'tmpl.php';
        }
        ...
        //run page builder in siteeditor and site|| site_editor_app_on()
        if ( !( is_site_editor()  || is_sed_save() ) ) {
            add_action('init', 'load_page_builder_app', 0);
        }

        if( site_editor_app_on() || is_sed_save() || isset( $_POST['sed_page_ajax']) ){
        ...
        //add_action( 'plugins_loaded', array( $this  , 'load_editor' ) , 10 );
        $this->load_editor();
    }
    ...
    function register_scripts(){
        wp_register_script( 'modernizr',SED_EDITOR_FOLDER_URL.'libraries/modernizr/modernizr.custom.min.js', array( ) );

        wp_register_script( 'handlebars',SED_EDITOR_FOLDER_URL.'libraries/handlebars/handlebars.min.js', array( ) );

        wp_register_script( 'sed-handlebars',SED_EDITOR_FOLDER_URL.'libraries/handlebars/sed.handlebars.min.js', array('handlebars' , 'jquery' , 'underscore' ) );

        wp_register_script( 'jquery-ui-full',SED_EDITOR_FOLDER_URL.'libraries/jquery/ui/full-js/jquery.ui.min.js', array( 'jquery' ) );

        //draggable overlap
        wp_register_script( 'sed-overlap',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/drag-drop/overlap.min.js', array( 'jquery-ui-full' ),"",1 );

        //draggable guidelines
        wp_register_script( 'sed-guidelines',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/drag-drop/guidelines.min.js', array( 'jquery-ui-full' ),"",1 );

        wp_register_script( 'jquery-contextmenu',SED_EDITOR_FOLDER_URL.'libraries/contextmenu/js/jquery.contextmenu.min.js', array( 'jquery','jquery-ui-full' , 'siteeditor-base' ) );

        wp_register_script( 'jquery-contenteditable', SED_EDITOR_FOLDER_URL.'libraries/jquery/jquery.contenteditable.min.js', array( 'jquery' ) , '1.0.0' );

        wp_register_script( 'jquery-livequery',SED_EDITOR_FOLDER_URL.'libraries/livequery/jquery.livequery.min.js', array( 'jquery','jquery-ui-full' ) );

        wp_register_script( 'sed-livequery',SED_EDITOR_FOLDER_URL.'libraries/livequery/sed.livequery.min.js', array( 'jquery-livequery' ) );

        wp_register_script( 'column-resize',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/column-resize.min.js', array( 'jquery','jquery-ui-full' ) );

        wp_register_script( 'siteeditor-base',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/siteeditor-base.min.js', array( 'jquery' ),"",1 );

        wp_register_script( 'siteeditor-shortcode',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/siteeditor-shortcode.min.js', array( 'siteeditor-base' , 'underscore' ,'jquery' ),"",1 );


        //plugins
        wp_register_script( 'delete-plugin',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/plugins/delete.min.js', array( 'siteeditor-base' , 'sed-app-preview' , 'sed-pagebuilder' ),"",1 );
        wp_register_script( 'select-plugin',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/plugins/select.min.js', array( 'siteeditor-base' ),"",1 );
        wp_register_script( 'media-plugin',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/plugins/media.min.js', array( 'siteeditor-base' , 'sed-app-preview' , 'sed-pagebuilder' ),"",1 );
        wp_register_script( 'preview-plugin',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/plugins/app-preview.min.js', array( 'siteeditor-base' , 'sed-app-preview' , 'site-iframe' ),"",1 );
        wp_register_script( 'duplicate-plugin',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/plugins/duplicate.min.js', array( 'siteeditor-base' , 'sed-app-preview' , 'sed-pagebuilder' ),"",1 );

        wp_register_script( 'siteeditor-modules-scripts',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/siteeditor-modules-scripts.min.js', array( 'siteeditor-base' ),"",1 );

        wp_register_script( 'siteeditor-ajax',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/siteeditor-ajax.min.js', array( 'siteeditor-base' ),"",1 );

        wp_register_script( 'tinycolor',SED_EDITOR_FOLDER_URL.'libraries/colorpicker/js/tinycolor.min.js', array( ),"",1 );

        wp_register_script( 'siteeditor-css',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/siteEditorCss.min.js', array( 'jquery', 'modernizr' , 'tinycolor' ),"",1 );

        wp_register_script( 'sed-app-preview',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/siteeditor-preview.min.js', array( 'jquery','siteeditor-base' , 'jquery-livequery' , 'underscore' ),"",1 );

        wp_register_script( 'sed-app-preview-render',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/siteeditor-preview-render.min.js', array( 'sed-app-preview' , 'siteeditor-css' ),"",1 );

        wp_register_script( 'sed-app-selective-refresh', SED_EDITOR_FOLDER_URL."libraries/siteeditor/site-iframe/sed-app-selective-refresh.min.js", array( 'jquery' , 'sed-app-preview' ), false, 1 );

        //wp_register_script( 'sed-style-editor', SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/style-editor.min.js', array( 'sed-app-preview-render' ) ,"",1 );

        //wp_register_script( 'sed-app-synchronization',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/siteeditor-synchronization.min.js', array( 'sed-app-preview-render' ),"",1 );

        wp_register_script( 'sed-app-shortcode-builder',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/shortcode-content-builder.min.js', array( 'sed-app-preview-render' ),"",1 );

        wp_register_script( 'site-iframe',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/site-iframe.min.js', array( 'sed-app-preview-render' , 'column-resize' , 'jquery-ui-full' , 'sed-app-shortcode-builder' ),"1.0.0" , 1 );

        wp_register_script( 'sed-pagebuilder',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/pagebuilder.min.js', array( 'site-iframe' , 'siteeditor-modules-scripts' , 'preview-plugin' ),"1.0.0" , 1 );

        wp_register_script( 'sed-module-free-draggable',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/drag-drop/sed-module-free-draggable.min.js', array( 'sed-pagebuilder' ),"1.0.0" , 1 );

        wp_register_script( 'sed-app-contextmenu-render',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/siteeditor-contextmenu.min.js', array( 'sed-app-preview-render' , 'jquery-contextmenu' ),"",1 );

        wp_register_script( 'sed-app-widgets',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/siteeditor-widgets.min.js', array( 'sed-pagebuilder' ),"",1 );
    ...
    function register_bootstrap_script(){
        wp_register_script( 'bootstrap-tooltip',SED_EDITOR_FOLDER_URL.'libraries/bootstrap/js/tooltip/tooltip.min.js', array( 'jquery'  ),"" );

        wp_register_script( 'bootstrap-popover',SED_EDITOR_FOLDER_URL.'libraries/bootstrap/js/popover/popover.min.js', array( 'bootstrap-tooltip' ),"" );
    }

    function register_theme_framework_scripts(){

        //images loaded
        wp_register_script( 'images-loaded',SED_PLUGIN_URL.'/framework/framework/js/imagesloaded/imagesloaded.pkgd.min.js', array() ,"1.2.4");

        wp_register_script('sed-app-site',SED_PLUGIN_URL.'/framework/framework/js/sed_app_site.min.js', array( 'jquery' ) , "1.0.0" );
        wp_register_script( 'wow-animate',SED_EDITOR_FOLDER_URL.'libraries/animate/js/wow.min.js', array( ) , "1.0.2" , 1 );
        wp_register_script('lightbox',SED_PLUGIN_URL.'/framework/framework/js/lightbox/lightbox.min.js', array( 'jquery' ) );

        wp_register_script( 'carousel',SED_PLUGIN_URL.'/framework/framework/js/slick.carousel/slick.min.js', array( ) ,"1.3.7");
        wp_register_script( 'easing',SED_PLUGIN_URL.'/framework/framework/js/easing/jquery.easing.1.3.js', array('jquery') ,"1.3");
        wp_register_script( 'packery',SED_PLUGIN_URL.'/framework/framework/js/metafizzy/packery.pkgd.min.js', array() ,"1.2.4");

        //deregister wordpress masonry
        wp_deregister_script( 'masonry' );
        wp_register_script( 'masonry',SED_PLUGIN_URL.'/framework/framework/js/masonry/masonry.pkgd.min.js', array() ,"1.2.4");
        wp_register_script( 'sed-masonry',SED_PLUGIN_URL.'/framework/framework/js/masonry/sed-masonry.min.js', array('masonry','sed-livequery' , 'images-loaded') ,"1.2.4");

        wp_register_script( 'isotope',SED_PLUGIN_URL.'/framework/framework/js/isotope/isotope.pkgd.min.js', array() ,"2.2.0");

        wp_register_script( 'waypoints',SED_PLUGIN_URL.'/framework/framework/js/waypoints/waypoints.js', array('jquery') ,"2.0.5");
        wp_register_script( 'jquery-parallax',SED_EDITOR_FOLDER_URL.'libraries/parallax/jquery.parallax.min.js', array( 'jquery' ) , "1.1.3" , 1 );

        wp_register_script( 'sed-ajax-load-posts',SED_PLUGIN_URL.'/framework/framework/js/post.ajax/sed-ajax-load-posts.min.js', array( ) , "1.0.0" , 1 );

        wp_register_script( 'sed-tinymce',SED_EDITOR_FOLDER_URL.'libraries/tinymce/tinymce.min.js', array() ,"4.0.5");
        /* REGISTER SCRIPT BOOTSTRAP
        ============================*/
        wp_register_script( 'bootstrap-tab',SED_PLUGIN_URL.'/framework/framework/js/bootstrap/bootstrap-tab.min.js', array('jquery') ,"3.3.2");
        wp_register_script( 'bootstrap-dropdown',SED_PLUGIN_URL.'/framework/framework/js/bootstrap/bootstrap-dropdown.js', array('jquery') ,"3.3.2");

        wp_register_script('render-scripts',SED_PLUGIN_URL.'/framework/framework/js/render.min.js', array( 'jquery' , 'wow-animate' , 'jquery-parallax' ) , "1.0.0" , 1 );
        /* REGISTER SCRIPT jplayer
        ========================*/
        wp_register_script( 'jplayer-plugin',SED_PLUGIN_URL.'/framework/framework/js/jplayer/jquery.jplayer.js', array('jquery') ,"2.7.0");
        wp_register_script( 'jplayer-playlist',SED_PLUGIN_URL.'/framework/framework/js/jplayer/jplayer.playlist.js', array('jquery','jplayer-plugin') ,"2.4.0");

        //scrollbar
        wp_register_script( 'custom-scrollbar', SED_EDITOR_FOLDER_URL.'libraries/scrollbar/js/jquery.mCustomScrollbar.concat.min.js', array('jquery'), '2.3' );

        /* REGISTER SCRIPT JQUERY UI
        ==========================*/
        /*wp_register_script( 'jquery-ui-core',SED_PLUGIN_URL.'/framework/framework/js/jquery/ui/jquery.ui.core.min.js', array('jquery') ,"1.10.4");
        wp_register_script( 'jquery-ui-widget',SED_PLUGIN_URL.'/framework/framework/js/jquery/ui/jquery.ui.widget.min.js', array('jquery','ui-core') ,"1.10.4");
        wp_register_script( 'jquery-effects-core',SED_PLUGIN_URL.'/framework/framework/js/jquery/ui/jquery.ui.effect.min.js', array('jquery','jquery-ui-core') ,"1.10.4");

        wp_register_script( 'jquery-ui-tabs',SED_PLUGIN_URL.'/framework/framework/js/jquery/ui/jquery.ui.tabs.min.js', array('jquery','ui-core','ui-widget') ,"1.10.4");
        wp_register_script( 'jquery-ui-accordion',SED_PLUGIN_URL.'/framework/framework/js/jquery/ui/jquery.ui.accordion.min.js', array('jquery','jquery-ui-core','jquery-ui-widget','jquery-effects-core') ,"1.10.4");
        */
    }

    function register_theme_framework_styles(){
        wp_register_style( 'css3-animate',SED_EDITOR_FOLDER_URL.'libraries/animate/css/animate.min.css' );
        wp_register_style( 'lightbox',SED_PLUGIN_URL.'/framework/framework/css/lightbox/lightbox.min.css' );
        wp_register_style( 'carousel',SED_PLUGIN_URL.'/framework/framework/css/slick.carousel/slick.css' ,"1.3.7");

        wp_register_style( 'general', SED_PLUGIN_URL.'/framework/framework/css/general.css' );
        wp_register_style( 'bootstrap-popover',SED_EDITOR_FOLDER_URL.'libraries/bootstrap/css/popover/popover.css');

        //scrollbar
        wp_register_style( 'custom-scrollbar', SED_EDITOR_FOLDER_URL.'libraries/scrollbar/css/jquery.mCustomScrollbar.css', array(), '2.3' );

        /* REGISTER STYLE FOR jplayer
        ============================*/
        //wp_register_style( 'jplayer-audio',SED_PLUGIN_URL.'/framework/framework/css/fonts-icon/font-awesome-4.2.0/css/font-awesome.min.css' ,"4.2.0");
    }
    ...
    function register_styles(){
        // wp_register_style( 'jquery-ui-full',SED_EDITOR_FOLDER_URL.'libraries/jquery/ui/css/jquery-ui.min.css' );

        wp_register_style( 'site-iframe',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/site-iframe.min.css' );

        wp_register_style( 'contextmenu',SED_EDITOR_FOLDER_URL.'libraries/contextmenu/css/style.min.css' );

        //wp_register_style( 'font-line-icon',SED_EDITOR_FOLDER_URL.'libraries/siteeditor/site-iframe/simple-line-icons.css' );
        wp_register_style( 'fonts-sed-iframe',SED_EDITOR_FOLDER_URL.'templates/default/css/fonts-sed-iframe.css' );
    }
    ...
    function sed_add_dynamic_css_file( ) {
        ...
        ob_start();
        include SED_PLUGIN_DIR . DS . 'framework' . DS . 'framework' . DS . 'dynamic_css.php';
