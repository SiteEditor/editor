...
final class SiteEditorCustomizePostsPreview {
	...
	public function preview_setup_postdata( WP_Post $post ) {
		...
		$setting_id = SiteEditorPostSetting::get_post_setting_id( $post );
		$setting = $this->component->manager->get_setting( $setting_id );
		if ( $setting instanceof SiteEditorPostSetting ) {
	...
	public function filter_the_posts_to_add_dynamic_post_settings_and_sections( array $posts ) {
		foreach ( $posts as &$post ) {
			$post_setting_id = SiteEditorPostSetting::get_post_setting_id( $post );
			...
			if ( $setting instanceof SiteEditorPostSetting ) { //&& ! $this->component->manager->get_section( $setting->id )
	...
	public function filter_the_posts_to_preview_settings( array $posts ) {
		foreach ( $posts as &$post ) {
			$post_setting_id = SiteEditorPostSetting::get_post_setting_id( $post );
			$setting = $this->component->manager->get_setting( $post_setting_id );
			if ( $setting instanceof SiteEditorPostSetting && isset( $this->previewed_post_settings[ $post->ID ] ) ) {
				$setting->override_post_data( $post );
			}
		}
		return $posts;
	...
	public function get_previewed_posts_for_query( WP_Query $query, $published = true ) {
		...
		if ( ! empty( $settings ) ) {
			foreach ( (array) $settings as $id => $post_data ) {
				if ( ! preg_match( SiteEditorPostSetting::SETTING_ID_PATTERN, $id, $matches ) ) {
					continue;
				}
	...
	public function filter_customize_dynamic_partial_args( $args, $id ) {
		if ( preg_match( WP_Customize_Post_Field_Partial::ID_PATTERN, $id, $matches ) ) {
			...
			$args['option_type'] = WP_Customize_Post_Field_Partial::TYPE;
	...
	function filter_get_edit_post_link( $url, $post_id ) {
		...
		$setting_id = SiteEditorPostSetting::get_post_setting_id( $edit_post );
