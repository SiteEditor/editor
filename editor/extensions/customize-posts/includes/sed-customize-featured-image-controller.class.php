...
/**
 * Class SiteEditorFeaturedImageController
 */
class SiteEditorFeaturedImageController extends SiteEditorPostmetaController {
		...
		$exports = array(
			'l10n' => array(
				'default_button_labels' => array(
					'change' => __( 'Change Image', 'site-editor' ),
					'default' => __( 'Default', 'site-editor' ),
					'placeholder' => __( 'No image selected', 'site-editor' ),
					'remove' => __( 'Remove', 'site-editor' ),
					'select' => __( 'Select Image', 'site-editor' ),
				),
			),
		);
	...
	public function handle_ajax_set_post_thumbnail() {
		...
		if ( false === $new_thumbnail_id ) {
			$return = '<p>' . esc_html__( 'Error: Invalid attachment selected.', 'site-editor' ) . '</p>';
	...
	public function filter_admin_post_thumbnail_html( $content, $post_id, $thumbnail_id = null ) {
		...
		$content .= '<p><strong>' . esc_html__( 'Note: The chosen image will not persist until you save.', 'site-editor' ) . '</strong></p>';
	 ...
	 *
	 * @global SiteEditorManager $wp_customize
	 *
	...
	public function add_partials() {
		...
		foreach ( $wp_customize->settings() as $setting ) {
			if ( $setting instanceof SiteEditorPostmetaSetting && $this->meta_key === $setting->meta_key ) {
				$partial_id = $setting->id;
				$partial_args = array(); // Note this is populated via SiteEditorFeaturedImageController::filter_customize_dynamic_partial_args().
	...
	public function filter_customize_dynamic_partial_args( $partial_args, $partial_id ) {
		if ( preg_match( SiteEditorPostmetaSetting::SETTING_ID_PATTERN, $partial_id, $matches ) && $matches['meta_key'] === $this->meta_key ) {
	...
	public function filter_post_thumbnail_html( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
		...
		$partial_id = SiteEditorPostmetaSetting::get_post_meta_setting_id( $post, $this->meta_key );
	 ...
	 */
	public function sanitize_setting( $attachment_id, SiteEditorPostmetaSetting $setting ) {
		unset( $setting );
		...
		/*
		 * Note that at this point, sanitize_meta() has already been called in SiteEditorPostmetaSetting::sanitize(),
		 * and the meta is registered wit SiteEditorFeaturedImageController::sanitize_value() as the sanitize_callback().
		 * So $attachment_id is either a valid attachment ID, -1, or false.
		 */
		if ( ! $is_valid  ) {
			return $has_setting_validation ? new WP_Error( 'invalid_attachment_id', __( 'The attachment is invalid.', 'site-editor' ) ) : null;
		}
