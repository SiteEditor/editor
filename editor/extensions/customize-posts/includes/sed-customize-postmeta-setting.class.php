<?php
/**
 * SiteEditor Customize Postmeta Setting class.
 *
 * @package WordPress
 * @subpackage Post Customize
 */

/**
 * Class SiteEditorPostmetaSetting
 */
class SiteEditorPostmetaSetting extends SedAppSettings {
	...
	/**
	 * SedAppSettings constructor.
	 *
	 * @access public
	 *
	 * @param SiteEditorManager $manager Manager.
	...
	public function __construct( SiteEditorManager $manager, $id, $args = array() ) {
		...
		if ( empty( $manager->posts ) || ! ( $manager->posts instanceof SiteEditorCustomizePosts ) ) {
	...
	public function sanitize( $meta_value ) {
		$has_setting_validation = method_exists( 'SedAppSettings', 'validate' );
		...
		if ( $this->single ) {
			...
			if ( ! is_array( $meta_value ) ) {
				return $has_setting_validation ? new WP_Error( 'expected_array', sprintf( __( 'Expected array value for non-single "%s" meta.', 'site-editor' ), $meta_key ) ) : null;
			}
			$values = $meta_value;
		}
		...
		foreach ( $values as &$value ) {
			...
			$value = apply_filters( "sed_app_sanitize_{$this->id}", $value, $this );
	 ...
	 *
	 * Note that the previewing logic is handled by SiteEditorCustomizePosts_Preview.
	 *
	 * @access public
	 *
	 * @return bool
