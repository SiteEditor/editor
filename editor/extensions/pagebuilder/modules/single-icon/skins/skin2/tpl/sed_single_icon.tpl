<# if( link ){ #>
    ...
    <a href="{{link}}" target="{{link_target}}" {{sed_attrs}} class="s-tb-sm ta-c sed-icons module module-single-icon single-icon-skin2 {{className}} <# if( style ){ #> {{hover_effect}} <# } #> " style="font-size:{{font_size}}px;">
...
<# }else{ #>
    ...
    <div {{sed_attrs}} class="s-tb-sm ta-c sed-icons module module-single-icon single-icon-skin2 {{className}} <# if( style ){ #> {{hover_effect}} <# } #>" style="font-size:{{font_size}}px;">
        <span class="hi-icon {{icon}} {{type}} {{style}}" sed-icon="{{icon}}" style="font-size:{{font_size}}px;color:{{color}}">
        </span>
    </div>
