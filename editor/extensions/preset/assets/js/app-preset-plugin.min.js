        ...
        */
			...
			api.addFilter('sedDefaultPatternsFilter', function ( patterns ) {
				...
				if( !_.isEmpty( self.defaultPresetsChanges ) ) {
					$.each(self.defaultPresetsChanges, function (shortcode_name, content) {
						...
						var newContent = api.sedShortcode.clone( content );

						//newContent.shift();

						//newContent.shift();

						patterns[shortcode_name] = newContent;
		...
		getCurrentContent : function () {
			...
			var mainShortcode = api.sedShortcode.getShortcode( api.currentTargetElementId ),
				...
				moduleSh = api.sedShortcode.getShortcode( parentId ) ,
				rowSh = api.sedShortcode.getShortcode( moduleSh.parent_id ),
				shortcodes = api.sedShortcode.findModelsById( collection , rowSh.id );

			var cloneShortcodes = api.sedShortcode.clone( shortcodes );

			//TODO : Remove module_helper_id
			cloneShortcodes = _.map( cloneShortcodes , function( shortcode ){

				if( !_.isUndefined(shortcode.attrs) && !_.isUndefined(shortcode.attrs.module_helper_id) ) {
	...
	api.PresetView = Backbone.View.extend({
		...
		setToDefault : function () {
			...
			if( isDefault ) {
				...
				//TODO : Support Row Wrapper For Set AS Default of Presets
				var newContent = api.sedShortcode.clone( content );
				...
				api.appPreset.defaultPresetsChanges[shortcodeName] = newContent;
