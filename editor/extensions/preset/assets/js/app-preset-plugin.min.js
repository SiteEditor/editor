...
(function( exports, $ ){
	...
	api.AppPreset = api.Class.extend({
        ...
        */
		...
		getCurrentContent : function () {
			...
			var mainShortcode = api.sedShortcode.getShortcode( api.currentTargetElementId ),
				helperShortcodes = [];

			$.each( collection , function(index , shortcode){
				if( shortcode.parent_id == mainShortcode.parent_id && !_.isUndefined(shortcode.attrs) && !_.isUndefined(shortcode.attrs.module_helper_id) && shortcode.attrs.module_helper_id == mainShortcode.id ){

					var copyModel = $.extend( true, {} , shortcode );

					copyModel.attrs.is_helper_id = true;

					delete copyModel.attrs.module_helper_id;

					helperShortcodes.push( copyModel );

				}
			});

			var allHelperShortcodes = [];

			_.each( helperShortcodes , function( shortcode ){

				var children = api.sedShortcode.findChildren( collection , shortcode.id );

				allHelperShortcodes.push( shortcode );

				if( !_.isEmpty( children ) ){

					allHelperShortcodes = $.merge( allHelperShortcodes , api.sedShortcode.clone( children ) );
			...
			if( !_.isEmpty( allHelperShortcodes ) ){

				cloneShortcodes = $.merge( cloneShortcodes , allHelperShortcodes );

			}

			console.log( "-----------cloneShortcodesPresets----------" , cloneShortcodes );
		...
		sync: function( method, model, options ) { 
            ...
            if ( 'create' === method ) {
                ...
                options.data = _.extend( options.data || {}, {
                    ...
                    nonce           : api.settings.nonce.preset.create
                });
            ...
            if ( 'read' === method ) {
				...
				options.data = _.extend( options.data || {}, {
                    ...
                    nonce           : api.settings.nonce.preset.get
				});
			...
			// This will permanently delete an attachment.
				...
				if( this.get("isDefault") ) {
					...
					api.previewer.send('sedSetAsDefaultPreset', {
						 name			: this.get("shortcode"),
						 content		: api.defaultPatterns[this.get("shortcode")] ,
					});
        ...
        sync: function( method, collection, options ) {
            ...
            if ( 'read' === method ) {
                ...
                options.data = _.extend( options.data || {}, {
                    ...
                    nonce           : api.settings.nonce.preset.collection
                });
                ...
                options.data = _.extend( options.data || {}, {
                    ...
                    nonce           : api.settings.nonce.preset.saveCollection
                });
	...
	api.PresetView = Backbone.View.extend({
		...
		load : function () {
			...
			var _loadPatternId = this.getNewLoadPatternId();

			/**
			 * load All Attachment Ids of a preset
			 */
			var attachment_ids = this.model.get("attachment_ids");

			if( $.isArray( attachment_ids ) && !_.isEmpty( attachment_ids ) ) {

				var options = {};

				options.context = this;
				options.url = api.settings.url.ajax;
				options.dataType = 'json';
				...
				api.wpAjax.send(options).done( function( resp ) { 

					_.each( resp , function( attachment ){
					...
					api.previewer.send( "sedLoadPresetAttachment" , {
						attachments 	: resp ,
						loadPatternId	: _loadPatternId
					});
			 ...
			 * Load Pattern
			 */
			api.previewer.send( "sedLoadNewPreset" , {
		...
		getNewLoadPatternId : function(){

			_loadPatternIdCounter++;

			return "sed_load_pattern_id_" + _loadPatternIdCounter;
	...
	api.SedAppPresetView = Backbone.View.extend({
		...
		addPreset : function(){
			...
			var content = api.appPreset.getCurrentContent(),
				attachmentIds = api.sedShortcode.getPatternAttachmentIds( content );
