 ...
 * @since 4.5.0
 */
final class SiteEditorSelectiveRefresh {
	 ...
	 * @since 4.5.0
	 */
	const RENDER_QUERY_VAR = 'sed_app_render_partials';
	...
	/**
	 * Registered instances of SiteEditorPartial.
	 *
	 * @since 4.5.0
	 * @access protected
	 * @var SiteEditorPartial[]
	 */
	 ...
	 * @access public
	 *
	 * @param SiteEditorManager $manager Manager instance.
	 */
	public function __construct( SiteEditorManager $manager ) {
		$this->manager = $manager;
		require_once SED_INC_EDITOR_DIR . DS . 'site-editor-partial.class.php';

		add_action( 'sed_app_preview_init', array( $this, 'init_preview' ) );
	}
	 ...
	 * @return SiteEditorPartial|null The partial, if set. Otherwise null.
	 */
	...
	public function enqueue_preview_scripts() {
		wp_enqueue_script( 'sed-app-selective-refresh' );
	...
	public function export_preview_data() {
		...
		// Export data to JS.
		echo sprintf( '<script>var _sedAppPartialRefreshExports = %s;</script>', wp_json_encode( $exports ) );
	}
	 ...
	 * @return array Added SiteEditorPartial instances.
	 */
	public function add_dynamic_partials( $partial_ids ) {
		...
		foreach ( $partial_ids as $partial_id ) {
			...
			$partial_args = false;
			$partial_class = 'SiteEditorPartial';
			 ...
			 * the SiteEditorPartial constructor.
			 *
			 * @since 4.5.0
			 *
			 * @param false|array $partial_args The arguments to the SiteEditorPartial constructor.
			 * @param string      $partial_id   ID for dynamic partial.
			 */
			$partial_args = apply_filters( 'sed_app_dynamic_partial_args', $partial_args, $partial_id );
			 ...
			 *
			 * Allow non-statically created partials to be constructed with custom SiteEditorPartial subclass.
			 *
			 * @since 4.5.0
			 *
			 * @param string $partial_class SiteEditorPartial or a subclass.
			 * @param string $partial_id    ID for dynamic partial.
			 * @param array  $partial_args  The arguments to the SiteEditorPartial constructor.
			 */
			$partial_class = apply_filters( 'sed_app_dynamic_partial_class', $partial_class, $partial_id, $partial_args );
	...
	public function handle_render_partials_request() {
		...
		/*
		 * Note that is_site_editor_preview() returning true will entail that the
		 * user passed the 'customize' capability check and the nonce check, since
		 * SiteEditorManager::setup_theme() is where the previewing flag is set.
		 */
		if ( ! is_site_editor_preview() ) {
			status_header( 403 );
			wp_send_json_error( 'expected_sed_app_preview' );
		 ...
		 * @since 4.5.0
		 *
		 * @param SiteEditorSelectiveRefresh $this     Selective refresh component.
		 * @param array                          $partials Placements' context data for the partials rendered in the request.
		 *                                                 The array is keyed by partial ID, with each item being an array of
		 *                                                 the placements' context data.
		 */
		do_action( 'sed_app_render_partials_before', $this, $partials );
		 ...
		 * via the {@see 'sed_app_render_partials_response'} filter.
		 *
		 * @since 4.5.0
		 *
		 * @param SiteEditorSelectiveRefresh $this     Selective refresh component.
		 ...
		 *                                                 the placements' context data.
		 */
		do_action( 'sed_app_render_partials_after', $this, $partials );
		 ...
		 *                                                 the placements' context data.
		 */
		$response = apply_filters( 'sed_app_render_partials_response', $response, $this, $partials );
