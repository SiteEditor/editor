/*#js_info#(
'handle'    => 'grid-gallery-default',
'deps'      => array('jquery','grid-gallery-default-plugins'),
'ver'       => '0.3.3',
'in_footer' => true
)#
*/
...
(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        init: function() {
            ...
            var _smallItemHeight = function() {

                var $heightSmall  = smallItem.width(),
                    $imgContainer = smallItem.find(".module-image");
                smallItem.css({
                    height   : $heightSmall
                });
                $imgContainer.css({
        		...
        		var _getImageDim = function( src ) {
                    ...
                    var c_w     = smallItem.find(".module-image").outerWidth(),
        				c_h		= smallItem.find(".module-image").outerHeight(),
        				r_w		= c_h / c_w,
        		...
        		var _setImagesSize = function() {
        			...
        			smallItems.find("img").each( function( i ) {
                ...
                */
                ...
                if( $element.parents(".module.module-portfolio:first").length > 0 ){
                    $element = $element.parents(".module.module-portfolio:first");
                }
                ...
                for column resize
                */
                $element.on("sed.moduleResize sed.moduleResizeStop sed.moduleSortableStop sedAfterRemoveColumns" , function(){
                   _smallItemHeight();
                   _setImagesSize();
                });


                /*$element.on("sed.portfolioAjaxLoaded" , function(){
                   _smallItemHeight();
                   _setImagesSize();
                });*/

                $element.on("sed.refreshGridGallery" , function(){
                   _smallItemHeight();
                   _setImagesSize();
                });
        ...
        bindListHandler: function(smallItems){
            ...
            if( this.options.expandModeEnabled ){
                smallItems.find('li').click( function(e){
                    e.preventDefault();
                    var $this = $(this);
...
jQuery(document).ready(function($){
	$('.sed-grid-gallery-default').livequery(function(){

	    var data = $(this).data();
                         
        $(this).sGallery({
            fullScreenEnabled : false,//data.fullScreenEnabled ,
            expandModeEnabled : data.expandModeEnabled
        });
