    ...
    public static function image_field( $pkey , $param , $in_box = true ){
        ...
        extract( array_merge(
                array(
                    ...
                    'value'      =>  SED_BASE_URL.'images/no-image.jpg',
                    'remove_btn' => false
                ),$param )
        );

        $value = (!empty($value)) ? $value : SED_BASE_URL.'images/no-image.jpg';

        $output  = '<div class="setting-img"><div class="change-img-setting">';
        $output .= '<div class="change-img-container"><img class="change_img" src="'.$value.'"/></div></div>';
        ...
        if($remove_btn === true)
            $output .= '<button class="remove-img-btn sed-btn-red">remove image</button>';
